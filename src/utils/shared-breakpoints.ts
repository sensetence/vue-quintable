...
let instance: SharedBreakpoints | null = null;

type BreakpointCallback = (hidden: string[]) => void;

class SharedBreakpoints {
  _subscribers: Set<BreakpointCallback>;
  _mqls: Record<string, MediaQueryList>;
  ...
  subscribe(cb: BreakpointCallback) {
    this._subscribers.add(cb);
    cb(this.hiddenBreakpoints);
  }

  unsubscribe(cb: BreakpointCallback) {
...
export function getSharedBreakpoints(): SharedBreakpoints {
  if (!instance) {
