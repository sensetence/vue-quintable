...
describe("checkListener (lines 3190-3216)", () => {
  it("checkListener sets allSelectedProperty false when not all selected", () => {
    ...
    w.vm.selected[1] = false;
    w.vm.selected[2] = false;
...
describe("localStorage exception handling (lines 1586-1589)", () => {
  it("storeState disabled when localStorage throws", async () => {
    const { _resetLocalStorageCache } = await import(
      "../src/components/table/vue-quintable.vue"
    );
